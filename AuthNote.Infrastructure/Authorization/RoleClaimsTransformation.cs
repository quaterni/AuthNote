using AuthNote.Infrastructure.Authentication;
...
namespace AuthNote.Infrastructure.Authorization
{
    internal class RoleClaimsTransformation : IClaimsTransformation
    {
        ...
        public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
        {
            ...
            var authrorizationService = scope.ServiceProvider.GetRequiredService<AuthorizationService>();

            var identityId = principal.GetIdentityId();

            var userRoles = await authrorizationService.GetRolesForUserAsync(identityId);
